...
from models import (
    ...
    CierreDiarioAlmacen,
    RepostajeVehiculo, ScanEvento, ScanNotificacion,
...
def mostrador_solicitudes_activas(trabajador_id: int, request: Request,
    ...
    for s in q.order_by(SolicitudTrabajador.creado_en.asc()).limit(20).all():
        salida.append({
            ...
            "voy": s.voy_a_recoger_en.strftime("%H:%M") if s.voy_a_recoger_en else "",
            "fotos": [f"/uploads/{f}" for f in s.fotos_lista],
        })
...
def _avisos_trabajador_util(db: Session, hoy: date | None = None) -> int:
    ...
    # 4) Revisión de la herramienta que lleva (mejora 14): fecha de próximo mantenimiento o
    #    mantenimiento programado en 7 días o ya pasado (hasta 60 días).
    def _quien_la_lleva(h):
        if h.responsable_id:
            return h.responsable_id
        mov = ultima.get(h.id)
        return mov.trabajador_id if (mov is not None and mov.trabajador_id) else None

    pronto = hoy + timedelta(days=7)
    for h in fuera:
        fpm = h.fecha_proximo_mantenimiento
        if not fpm or fpm > pronto or fpm < hoy - timedelta(days=60):
            continue
        tid = _quien_la_lleva(h)
        t = db.get(Trabajador, tid) if tid else None
        if t is None or not t.activo:
            continue
        f = fpm.strftime("%d/%m/%Y")
        if fpm >= hoy:
            aviso(tid, f"Revisión de {h.nombre} el {f}", f"A {h.nombre} ({h.codigo or ''}) le toca revisión el {f}. Llévala al almacén antes de ese día; desde tu portal puedes pedir la devolución con un toque.", f"revision:{h.id}:{fpm.isoformat()}", "#asignado", "revision")
        else:
            aviso(tid, f"Revisión vencida: {h.nombre}", f"A {h.nombre} ({h.codigo or ''}) le tocaba revisión el {f}. Llévala al almacén cuanto antes.", f"revision:{h.id}:vencida:{fpm.isoformat()}", "#asignado", "revision")
    fuera_ids = {h.id: h for h in fuera}
    if fuera_ids:
        for mp in db.query(MantenimientoProgramado).filter(
            MantenimientoProgramado.tipo_activo == "herramienta", MantenimientoProgramado.activo_id.in_(list(fuera_ids)),
            MantenimientoProgramado.estado.in_(("pendiente", "programado")),
            MantenimientoProgramado.fecha_programada <= datetime.combine(pronto, datetime.max.time()),
            MantenimientoProgramado.fecha_programada >= datetime.combine(hoy - timedelta(days=60), datetime.min.time()),
        ).all():
            h = fuera_ids.get(mp.activo_id)
            tid = _quien_la_lleva(h) if h is not None else None
            t = db.get(Trabajador, tid) if tid else None
            if h is None or t is None or not t.activo:
                continue
            f = mp.fecha_programada.strftime("%d/%m/%Y")
            aviso(tid, f"{h.nombre}: {mp.tipo or 'mantenimiento'} el {f}", f"Hay un mantenimiento programado para {h.nombre} ({h.codigo or ''}) el {f}{(': ' + mp.descripcion) if mp.descripcion else ''}. Llévala al almacén.", f"mant:{mp.id}:{mp.fecha_programada.date().isoformat()}", "#asignado", "revision")

    # 5) Revisión mensual del EPI (mejora 7): a quien tiene EPI y no lo ha revisado en 30 días.
    mes = hoy.strftime("%Y-%m")
    con_epi = {tid for (tid,) in db.query(EPIIndividual.trabajador_id).filter(EPIIndividual.trabajador_id.isnot(None), EPIIndividual.estado == "activo").distinct().all()}
    con_epi |= {tid for (tid,) in db.query(EntregaEPI.trabajador_id).filter(EntregaEPI.fecha >= hoy - timedelta(days=365)).distinct().all()}
    for t in db.query(Trabajador).filter(Trabajador.activo == True, Trabajador.portal_token.isnot(None), Trabajador.id.in_(list(con_epi)) if con_epi else False).all():
        if t.epi_revisado_en and t.epi_revisado_en.date() >= hoy - timedelta(days=30):
            continue
        aviso(t.id, "Revisa tu EPI este mes", "Entra en «Tu EPI» y marca con un toque si cada pieza está bien o hay que cambiarla. Lo que falte se pide solo.", f"epirev:{t.id}:{mes}", "#tu-epi", "epi")

    # 6) Lo que lleva mucho tiempo fuera sin plazo (mejora 15): cada 30 días pregunta si la sigue necesitando.
    #    No es un aviso de "no ha vuelto": solo una pregunta amable con dos botones en el portal.
    for h in fuera:
        mov = ultima.get(h.id)
        if mov is None or mov.fecha_devolucion_prevista or not mov.fecha:
            continue
        tid = h.responsable_id or mov.trabajador_id
        if not tid:
            continue
        dias_fuera = (hoy - _utc_a_local(mov.fecha).date()).days
        if dias_fuera < 30:
            continue
        if h.confirmada_portal_en and (hoy - h.confirmada_portal_en.date()).days < 30:
            continue
        t = db.get(Trabajador, tid)
        if t is None or not t.activo:
            continue
        periodo = dias_fuera // 30
        aviso(tid, f"¿Sigues necesitando {h.nombre}?", f"Llevas {dias_fuera} días con {h.nombre} ({h.codigo or ''}). Si la sigues usando, dilo con un toque en «Lo que tengo»; si no, pide la devolución y el almacén la recoge.", f"fuera:{h.id}:{periodo}", "#asignado", "aviso")
...
def portal_trabajador(token: str, request: Request, db: Session = Depends(get_db)):
    ...
    # Pantalla "Hoy" (mejora 1): lo que requiere su atención, con un toque a cada sección.
    hoy_items: list[dict] = []
    _hoy = date.today()
    _n_listas = [s for s in solicitudes if s.estado == "lista" and not s.recogida_confirmada_en]
    if _n_listas:
        hoy_items.append({"icono": "bi-bag-check-fill", "nivel": "ok", "enlace": "#solicitudes",
                          "texto": (f"{len(_n_listas)} pedidos listos para recoger" if len(_n_listas) > 1 else f"Tu pedido {_n_listas[0].numero} está listo para recoger")})
    _sin_firmar = [a for a in albaranes if a.tipo_documento != "entrada" and (a.portal_conformidad or "pendiente") == "pendiente"]
    if _sin_firmar:
        hoy_items.append({"icono": "bi-pen", "nivel": "warn", "enlace": f"/portal/{t.portal_token}/albaranes/{_sin_firmar[0].id}" if len(_sin_firmar) == 1 else "#albaranes",
                          "texto": (f"{len(_sin_firmar)} albaranes sin firmar" if len(_sin_firmar) > 1 else f"Albarán {_sin_firmar[0].numero} sin firmar")})
    _vencidas = [h for h in herramientas if h.get("vencida")]
    if _vencidas:
        hoy_items.append({"icono": "bi-calendar-x", "nivel": "danger", "enlace": "#asignado",
                          "texto": (f"{len(_vencidas)} herramientas con el plazo de devolución vencido" if len(_vencidas) > 1 else f"Plazo vencido: {_vencidas[0]['nombre']}")})
    _revisar = [h for h in herramientas if h.get("revision_pronto") and not h.get("vencida")]
    if _revisar:
        hoy_items.append({"icono": "bi-wrench-adjustable", "nivel": "warn", "enlace": "#asignado",
                          "texto": (f"{len(_revisar)} herramientas con revisión próxima" if len(_revisar) > 1 else f"Revisión de {_revisar[0]['nombre']} el {_revisar[0]['revision']}")})
    _no_confirmadas = [h for h in herramientas if not h.get("confirmada")]
    if _no_confirmadas and not _vencidas:
        hoy_items.append({"icono": "bi-check2-all", "nivel": "info", "enlace": "#asignado",
                          "texto": f"Confirma con un toque que tienes tus {len(_no_confirmadas)} herramientas" if len(_no_confirmadas) > 1 else f"Confirma que tienes {_no_confirmadas[0]['nombre']}"})
    for fo in formaciones:
        if fo.fecha_caducidad and fo.fecha_caducidad <= _hoy + timedelta(days=30):
            hoy_items.append({"icono": "bi-mortarboard", "nivel": "danger" if fo.fecha_caducidad < _hoy else "warn", "enlace": "#documentacion",
                              "texto": f"Formación «{fo.nombre_curso}» {'caducada el' if fo.fecha_caducidad < _hoy else 'caduca el'} {fo.fecha_caducidad.strftime('%d/%m/%Y')}"})
    for rm in reconocs[:1]:
        if rm.fecha_proxima and rm.fecha_proxima <= _hoy + timedelta(days=30):
            hoy_items.append({"icono": "bi-heart-pulse", "nivel": "warn", "enlace": "#documentacion",
                              "texto": f"Reconocimiento médico {'pendiente desde el' if rm.fecha_proxima < _hoy else 'el'} {rm.fecha_proxima.strftime('%d/%m/%Y')}"})
    if (epis or catalogo_kit_portal) and (not t.epi_revisado_en or t.epi_revisado_en.date() < _hoy - timedelta(days=30)):
        hoy_items.append({"icono": "bi-shield-check", "nivel": "info", "enlace": "#tu-epi", "texto": "Revisa tu EPI este mes (un toque por pieza)"})
    comunicados_portal = _comunicados_activos_para(db, t)   # mejora 17
    _por_leer = [c for c in comunicados_portal if c["obligatorio"] and not c["leido"]]
    if _por_leer:
...
# ─── Marcador de cuidado del material (mejora 18) ────────────────────────────

def _cuidado_material(db: Session, t: Trabajador, hoy: date | None = None) -> dict:
    """Puntos (0-100) y estrellas (0-5) de los últimos 90 días: devolver a tiempo, comunicar incidencias,
    revisar el EPI, confirmar lo que tiene y firmar recogidas. Empieza en 50."""
    hoy = hoy or date.today()
    desde = datetime.combine(hoy - timedelta(days=90), datetime.min.time())
    puntos, detalle = 50, []
    a_tiempo = tarde = 0
    devs = db.query(Movimiento).filter(Movimiento.trabajador_id == t.id, Movimiento.tipo == "devolucion", Movimiento.fecha >= desde, Movimiento.herramienta_id.isnot(None)).all()
    for d in devs:
        ent = db.query(Movimiento).filter(Movimiento.herramienta_id == d.herramienta_id, Movimiento.tipo.in_(["entrega", "traslado"]), Movimiento.id < d.id).order_by(Movimiento.id.desc()).first()
        if ent is None or not ent.fecha_devolucion_prevista:
            continue
        if _utc_a_local(d.fecha) <= ent.fecha_devolucion_prevista + timedelta(days=1):
            a_tiempo += 1
        else:
            tarde += 1
    if a_tiempo:
        p = min(30, 10 * a_tiempo); puntos += p; detalle.append((f"{a_tiempo} devolución(es) a tiempo", p))
    if tarde:
        p = -min(30, 10 * tarde); puntos += p; detalle.append((f"{tarde} devolución(es) tarde", p))
    vencidas = [h for h in _portal_lo_que_tengo(db, t) if h.get("vencida")]
    if vencidas:
        p = -min(30, 10 * len(vencidas)); puntos += p; detalle.append((f"{len(vencidas)} plazo(s) vencido(s) ahora mismo", p))
    n_inc = db.query(IncidenciaPortalTrabajador).filter(IncidenciaPortalTrabajador.trabajador_id == t.id, IncidenciaPortalTrabajador.creado_en >= desde).count()
    if n_inc:
        p = min(15, 5 * n_inc); puntos += p; detalle.append((f"{n_inc} incidencia(s) comunicada(s)", p))
    if t.epi_revisado_en and t.epi_revisado_en.date() >= hoy - timedelta(days=30):
        puntos += 15; detalle.append(("EPI revisado este mes", 15))
    n_conf = db.query(Herramienta).filter(Herramienta.responsable_id == t.id, Herramienta.confirmada_portal_en.isnot(None), Herramienta.confirmada_portal_en >= datetime.combine(hoy - timedelta(days=30), datetime.min.time())).count()
    if n_conf:
        puntos += 10; detalle.append(("Confirmó lo que tiene este mes", 10))
    n_firmas = db.query(SolicitudTrabajador).filter(SolicitudTrabajador.trabajador_id == t.id, SolicitudTrabajador.recogida_firma_datos.isnot(None), SolicitudTrabajador.recogida_confirmada_en >= desde).count()
    if n_firmas:
        p = min(10, 5 * n_firmas); puntos += p; detalle.append((f"{n_firmas} recogida(s) firmada(s) en el móvil", p))
    puntos = max(0, min(100, puntos))
    return {"puntos": puntos, "estrellas": int(round(puntos / 20)), "detalle": detalle}


@app.get("/trabajadores/cuidado", response_class=HTMLResponse)
def trabajadores_cuidado(request: Request, user: Usuario = Depends(requiere_login), db: Session = Depends(get_db)):
    """Ranking de cuidado del material (mejora 18)."""
    filas = []
    for t in db.query(Trabajador).filter(Trabajador.activo == True).order_by(Trabajador.nombre).all():
        c = _cuidado_material(db, t)
        filas.append({"t": t, "puntos": c["puntos"], "estrellas": c["estrellas"], "detalle": c["detalle"]})
    filas.sort(key=lambda f: (-f["puntos"], f["t"].nombre_completo))
...
def _portal_lo_que_tengo(db: Session, t: Trabajador) -> list[dict]:
    ...
    for h in hs:
        ...
        salida.append({
            ...
            "confirmada": h.confirmada_portal_en.strftime("%d/%m/%Y") if h.confirmada_portal_en else "",
            "confirmada_hace": (hoy - h.confirmada_portal_en.date()).days if h.confirmada_portal_en else None,   # mejora 15
            "foto": f"/static/uploads/herramientas/{h.foto_path}" if h.foto_path else "",   # mejora 8
            "revision": h.fecha_proximo_mantenimiento.strftime("%d/%m/%Y") if h.fecha_proximo_mantenimiento else "",   # mejora 14
            "revision_pronto": bool(h.fecha_proximo_mantenimiento and h.fecha_proximo_mantenimiento <= hoy + timedelta(days=7)),
            "ficha": f"/portal/{t.portal_token}/herramientas/{h.id}" if t.portal_token else "",
        })
    return salida
...
async def portal_crear_solicitud(token: str, request: Request, db: Session = Depends(get_db)):
    ...
    fotos_pedido = await _save_worker_portal_photos(form, trabajador.id, "pedido", max_n=3)   # mejora 11
    if not items and fotos_pedido:
        items = [{"tipo": (tipos[0] if tipos and tipos[0] in ("herramienta", "maquinaria", "ropa", "epi", "consumible", "otro") else "otro"), "descripcion": "Ver foto",
                  "talla": tallas[0] if tallas else "", "cantidad": cantidades[0] if cantidades else "1", "espera": False}]
    try:
        ...
        if fotos_pedido and not solicitud.fotos_json:
            solicitud.fotos_json = json.dumps(fotos_pedido)
        db.add(AuditoriaLog(
        ...
        try:
            ...
            aviso = Aviso(
                ...
                mensaje=f"{n_items} articulo(s) pedido(s)."
                        ...
                        + (f" Con {len(solicitud.fotos_lista)} foto(s)." if solicitud.fotos_lista else ""),
...
async def portal_crear_comunicacion(token: str, request: Request, db: Session = Depends(get_db)):
    ...
    audio_buzon = await _save_worker_portal_audio(form, trabajador.id, "voz_buzon")
    texto_buzon = str(form.get("mensaje") or "").strip()
    if audio_buzon and len(texto_buzon) < 10:
        texto_buzon = (texto_buzon + " (mensaje de voz adjunto)").strip()
    try:
        mensaje = create_worker_message(
            ...
            subject=str(form.get("asunto") or ""), message=texto_buzon,
            worksite=str(form.get("obra") or ""),
        )
        mensaje.audio_path = audio_buzon
        db.add(AuditoriaLog(
...
def _comunicados_activos_para(db: Session, t: Trabajador) -> list[dict]:
    """Avisos activos con si este trabajador ya los leyó."""
    leidos = {cid for (cid,) in db.query(ComunicadoLectura.comunicado_id).filter(ComunicadoLectura.trabajador_id == t.id).all()}
    salida = []
    for c in db.query(ComunicadoEmpresa).filter(ComunicadoEmpresa.activo == True).order_by(ComunicadoEmpresa.creado_en.desc()).limit(30).all():
        salida.append({"id": c.id, "titulo": c.titulo, "texto": c.texto, "obligatorio": bool(c.obligatorio),
                       "fecha": _utc_a_local(c.creado_en).strftime("%d/%m/%Y") if c.creado_en else "", "leido": c.id in leidos})
    return salida


@app.get("/comunicados", response_class=HTMLResponse)
def comunicados_lista(request: Request, user: Usuario = Depends(requiere_login), db: Session = Depends(get_db)):
    if user.rol != "admin" and not tiene_permiso(user, "crear"):
        raise HTTPException(403, "Sin permiso")
    trabajadores = db.query(Trabajador).filter(Trabajador.activo == True, Trabajador.portal_token.isnot(None)).order_by(Trabajador.nombre).all()
    filas = []
    for c in db.query(ComunicadoEmpresa).order_by(ComunicadoEmpresa.activo.desc(), ComunicadoEmpresa.creado_en.desc()).limit(100).all():
        leidos = {l.trabajador_id: l.leido_en for l in c.lecturas}
        filas.append({"c": c, "leidos": len([t for t in trabajadores if t.id in leidos]), "total": len(trabajadores),
                      "pendientes": [t.nombre_completo for t in trabajadores if t.id not in leidos]})
    return templates.TemplateResponse(request, "comunicados.html", ctx_base(request, user, db, filas=filas, total_trabajadores=len(trabajadores)))


@app.post("/comunicados", response_class=RedirectResponse)
def comunicados_crear(request: Request, titulo: str = Form(...), texto: str = Form(...), obligatorio: str = Form("1"),
                      user: Usuario = Depends(requiere_login), db: Session = Depends(get_db)):
    if user.rol != "admin" and not tiene_permiso(user, "crear"):
        raise HTTPException(403, "Sin permiso")
    titulo, texto = " ".join(titulo.split())[:200], texto.strip()[:5000]
    if len(titulo) < 3 or len(texto) < 3:
        raise HTTPException(422, "Escribe un título y el texto del aviso")
    c = ComunicadoEmpresa(titulo=titulo, texto=texto, obligatorio=(obligatorio not in ("0", "", "no")), creado_por_id=user.id)
    db.add(c)
    db.flush()
    n = 0
    for t in db.query(Trabajador).filter(Trabajador.activo == True, Trabajador.portal_token.isnot(None)).all():
        create_worker_notification(db, t.id, title=f"Aviso de la empresa: {titulo}", message=(texto[:300] + ("…" if len(texto) > 300 else "")) + (" Márcalo como leído en tu portal." if c.obligatorio else ""),
                                   kind="comunicado", link="#comunicados", event_key=f"comunicado:{c.id}:{t.id}")
        n += 1
    db.add(AuditoriaLog(tabla="comunicados_empresa", registro_id=c.id, accion="publicar", resumen=f"Aviso «{titulo}» publicado a {n} trabajadores", usuario_id=user.id))
    db.commit()
    return RedirectResponse("/comunicados?ok=1", status_code=303)


@app.post("/comunicados/{cid}/archivar", response_class=RedirectResponse)
def comunicados_archivar(cid: int, user: Usuario = Depends(requiere_login), db: Session = Depends(get_db)):
    if user.rol != "admin" and not tiene_permiso(user, "crear"):
        raise HTTPException(403, "Sin permiso")
    c = db.get(ComunicadoEmpresa, cid)
    if c is None:
        raise HTTPException(404, "Aviso no encontrado")
    c.activo = False
    db.commit()
    return RedirectResponse("/comunicados", status_code=303)


@app.post("/portal/{token}/comunicados/{cid}/leido", response_class=RedirectResponse)
def portal_comunicado_leido(token: str, cid: int, request: Request, db: Session = Depends(get_db)):
    worker = _portal_worker_required(token, request, db)
    c = db.get(ComunicadoEmpresa, cid)
    if c is None or not c.activo:
        raise HTTPException(404, "Aviso no encontrado")
    if not db.query(ComunicadoLectura).filter_by(comunicado_id=cid, trabajador_id=worker.id).first():
        db.add(ComunicadoLectura(comunicado_id=cid, trabajador_id=worker.id))
        db.add(AuditoriaLog(tabla="comunicados_empresa", registro_id=cid, accion="leido", resumen=f"{worker.nombre_completo} marcó leído «{c.titulo}»", usuario_id=None))
        db.commit()
    return RedirectResponse(f"/portal/{token}?ok=leido#comunicados", status_code=303)


@app.post("/portal/{token}/epi/revision", response_class=RedirectResponse)
async def portal_epi_revision(token: str, request: Request, db: Session = Depends(get_db)):
    """Revisión mensual del EPI con un toque (mejora 7): lo marcado como mal genera incidencia (con foto)
    y lo que falta del kit genera un pedido de reposición."""
    worker = _portal_worker_required(token, request, db)
    form = await request.form()
    detalle = str(form.get("detalle") or "").strip()[:1000]
    fotos = await _save_worker_portal_photos(form, worker.id, "epirev", max_n=5)
    mal_epis, faltan_kit = [], []
    for e in db.query(EPIIndividual).filter(EPIIndividual.trabajador_id == worker.id, EPIIndividual.estado == "activo").all():
        if str(form.get(f"epi_{e.id}") or "bien") == "mal":
            mal_epis.append(e)
    nombres = form.getlist("kit_nombre")
    for i, nombre in enumerate(nombres):
        nombre = " ".join(str(nombre).split())[:100]
        if nombre and str(form.get(f"kit_estado_{i}") or "bien") == "falta":
            faltan_kit.append(nombre)
    creados = []
    for e in mal_epis:
        inc = create_worker_incident(
            db, worker, category="seguridad", asset_type="epi",
            asset_code=(e.referencia_interna or e.codigo_qr or e.codigo_fabricacion or "")[:100], asset_name=f"{e.tipo} {e.marca or ''}".strip()[:200],
            description=("Revisión mensual de EPI desde el móvil: en mal estado." + (f" {detalle}" if detalle else ""))[:2000],
            photo_path=fotos[0] if fotos else None,
        )
        if fotos:
            inc.fotos_json = json.dumps(fotos)
        creados.append(inc.numero)
    if faltan_kit:
        sol = create_worker_request(
            db, worker, submission_id=f"epirev-{worker.id}-{datetime.now():%Y%m%d%H%M%S}", priority="normal",
            destination="", reason=("Reposición tras la revisión mensual de EPI desde el móvil." + (f" {detalle}" if detalle else ""))[:2000],
            items=[{"tipo": "epi", "descripcion": n, "talla": "", "cantidad": "1"} for n in faltan_kit],
        )
        creados.append(sol.numero)
    worker.epi_revisado_en = datetime.now()
    db.add(AuditoriaLog(tabla="trabajadores", registro_id=worker.id, accion="epi_revision_portal",
                        resumen=f"Revisión de EPI desde el portal: {len(mal_epis)} en mal estado, faltan {len(faltan_kit)} del kit" + (f" ({', '.join(creados)})" if creados else ""), usuario_id=None))
    if mal_epis or faltan_kit:
        db.add(Aviso(titulo=f"{worker.nombre_completo}: revisión de EPI con problemas",
                     mensaje=("En mal estado: " + ", ".join(e.tipo for e in mal_epis) + ". " if mal_epis else "") + ("Faltan del kit: " + ", ".join(faltan_kit) + "." if faltan_kit else ""),
                     prioridad="media", tipo="sistema", enlace="/operaciones-portal-trabajadores"))
    db.commit()
    return RedirectResponse(f"/portal/{token}?ok=epi_revisado#tu-epi", status_code=303)


@app.post("/portal/{token}/solicitudes/{request_id}/voy", response_class=RedirectResponse)
def portal_voy_a_recoger(token: str, request_id: int, request: Request, db: Session = Depends(get_db)):
    """'Voy a recogerlo' (mejora 9): el almacén sabe que el trabajador va de camino."""
    worker = _portal_worker_required(token, request, db)
    row = db.get(SolicitudTrabajador, request_id)
    if not row or row.trabajador_id != worker.id:
        raise HTTPException(404, "Solicitud no encontrada")
    if row.estado != "lista" or row.recogida_confirmada_en:
        raise HTTPException(409, "Ese pedido no está pendiente de recoger")
    if row.voy_a_recoger_en:
        raise HTTPException(409, "Ya avisaste de que vas a recogerlo")
    row.voy_a_recoger_en = datetime.now()
    db.add(Aviso(titulo=f"{worker.nombre_completo} va a recoger el pedido {row.numero}",
                 mensaje=f"Avisó desde su móvil a las {row.voy_a_recoger_en.strftime('%H:%M')}. Tenedlo a mano en el Mostrador.",
                 prioridad="media", tipo="sistema", enlace="/solicitudes-trabajadores"))
    db.add(AuditoriaLog(tabla="solicitudes_trabajador", registro_id=row.id, accion="voy_a_recoger",
                        resumen=f"{worker.nombre_completo} avisó que va a recoger {row.numero}", usuario_id=None))
    db.commit()
    return RedirectResponse(f"/portal/{token}?ok=voy#solicitudes", status_code=303)


@app.get("/portal/{token}/herramientas/{hid}", response_class=HTMLResponse)
def portal_ficha_herramienta(token: str, hid: int, request: Request, db: Session = Depends(get_db)):
    """Ficha de una herramienta que consta a nombre del trabajador (mejora 8): foto, datos,
    últimos movimientos, mantenimiento y manuales."""
    worker = _portal_worker_required(token, request, db)
    mias = {x["id"]: x for x in _portal_lo_que_tengo(db, worker)}
    if hid not in mias:
        raise HTTPException(404, "Esa herramienta no consta a tu nombre")
    h = db.get(Herramienta, hid)
    movs = []
    for m in db.query(Movimiento).filter(Movimiento.herramienta_id == hid).order_by(Movimiento.id.desc()).limit(10).all():
        quien = db.get(Trabajador, m.trabajador_id) if m.trabajador_id else None
        movs.append({"fecha": _utc_a_local(m.fecha).strftime("%d/%m/%Y %H:%M") if m.fecha else "", "tipo": (m.tipo or "").replace("_", " "),
                     "donde": m.destino or m.origen or "", "quien": quien.nombre_completo if quien else ""})
    docs = [d for d in db.query(Documento).filter(Documento.herramienta_id == hid).order_by(Documento.created_at.desc()).limit(10).all() if d.archivo]
    mantenimientos = db.query(MantenimientoProgramado).filter(
        MantenimientoProgramado.tipo_activo == "herramienta", MantenimientoProgramado.activo_id == hid,
        MantenimientoProgramado.estado.in_(("pendiente", "programado", "en_curso")),
    ).order_by(MantenimientoProgramado.fecha_programada.asc()).limit(3).all()
    loc = db.get(Ubicacion, h.ubicacion_id) if h.ubicacion_id else None
    response = templates.TemplateResponse(request, "portal_herramienta.html", {
        "request": request, "trabajador": worker, "h": h, "mia": mias[hid], "movs": movs, "docs": docs,
        "mantenimientos": mantenimientos, "hueco": (loc.ruta_completa or loc.nombre) if loc else (h.ubicacion_texto or ""),
        "t": _portal_traductor(worker.idioma), "idioma": (worker.idioma or "es"),
    })
    response.headers["Cache-Control"] = "no-store, private"
    return response


@app.get("/portal/{token}/documentos/{did}")
def portal_documento_herramienta(token: str, did: int, request: Request, db: Session = Depends(get_db)):
    """Descarga de un manual/certificado de una herramienta que consta a nombre del trabajador (mejora 8)."""
    worker = _portal_worker_required(token, request, db)
    doc = db.get(Documento, did)
    if doc is None or not doc.archivo or not doc.herramienta_id:
        raise HTTPException(404, "Documento no encontrado")
    if doc.herramienta_id not in {x["id"] for x in _portal_lo_que_tengo(db, worker)}:
        raise HTTPException(404, "Documento no encontrado")
    ruta = UPLOADS_DIR / "documentos" / os.path.basename(doc.archivo)
    if not ruta.is_file():
        raise HTTPException(404, "El archivo ya no está en el servidor")
    return FileResponse(str(ruta), filename=f"{doc.nombre or 'documento'}{('.' + doc.extension) if doc.extension and not (doc.nombre or '').endswith('.' + doc.extension) else ''}")
...
async def _save_worker_portal_audio(form, worker_id: int, prefix: str) -> str | None:
    """Mensaje de voz grabado en el móvil (mejora 10): campo `audio`, hasta 3 MB, webm/ogg/m4a/mp4/mp3/wav."""
    up = form.get("audio")
    if up is None or not getattr(up, "filename", ""):
        return None
    ext = (up.filename.rsplit(".", 1)[-1] if "." in up.filename else "").lower()[:5]
    ctype = (getattr(up, "content_type", "") or "").lower()
    if ext not in {"webm", "ogg", "m4a", "mp4", "mp3", "wav", "oga"}:
        ext = {"audio/webm": "webm", "video/webm": "webm", "audio/ogg": "ogg", "audio/mp4": "m4a", "audio/mpeg": "mp3", "audio/wav": "wav", "audio/x-m4a": "m4a"}.get(ctype.split(";")[0].strip(), "")
    if not ext:
    ...
    if len(data) > 3 * 1024 * 1024:
        raise HTTPException(400, "El mensaje de voz es demasiado largo (máximo 3 MB)")
    folder = UPLOADS_DIR / "portal_trabajador"
    folder.mkdir(parents=True, exist_ok=True)
    filename = f"{prefix}_{worker_id}_{uuid.uuid4().hex}.{ext}"
    (folder / filename).write_bytes(data)
...
async def portal_crear_incidencia(token: str, request: Request, db: Session = Depends(get_db)):
    ...
    audio = await _save_worker_portal_audio(form, worker.id, "voz_inc")
    photo = fotos[0] if fotos else None
    descripcion_inc = str(form.get("descripcion") or "").strip()
    if audio and len(descripcion_inc) < 10:
        descripcion_inc = (descripcion_inc + " (mensaje de voz adjunto)").strip()
    try:
        row = create_worker_incident(
            ...
            description=descripcion_inc, photo_path=photo,
        )
...
async def vehiculos_movimientos(
    ...
    # Repostajes en gasolinera. El antiguo módulo de combustible interno se eliminó en 2.7.31.
    repostajes = db.query(RepostajeVehiculo).order_by(RepostajeVehiculo.fecha.desc()).limit(150).all()
