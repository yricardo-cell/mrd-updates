...
def _dt(d):
    """Convierte date o datetime a datetime para aritmética segura."""
    if d is None:
        return None
...
def _detectar_herramientas(db) -> list[dict]:
    ...
    # Estar fuera es una operación normal. Solo se alerta cuando el usuario
    # indicó un plazo de devolución y ese plazo ya ha vencido.
    for h in herramientas:
        if h.estado in ("entregada", "en_obra"):
            entrega = db.query(Movimiento).filter(
                Movimiento.herramienta_id == h.id,
                Movimiento.tipo == "entrega",
            ).order_by(Movimiento.id.desc()).first()
            prevista = _dt(getattr(entrega, "fecha_devolucion_prevista", None)) if entrega else None
            if not prevista or prevista >= ahora:
                continue
            horas = max(1, int((ahora - prevista).total_seconds() // 3600))
            dias = (ahora - prevista).days
            demora = f"{horas} horas" if dias < 1 else f"{dias} días"
            anomalias.append(_anomalia(
                tipo="devolucion_fuera_de_plazo",
                severidad="critica" if dias >= 7 else ("alta" if dias >= 2 else "media"),
                descripcion=f"«{h.nombre}» ha superado en {demora} la devolución prevista.",
                item_id=h.id, item_nombre=h.nombre, item_tipo="herramienta",
                enlace=f"/herramientas/{h.id}",
                valor=horas, umbral=0, metodo="plazo_usuario",
            ))
...
def _detectar_reparaciones(db) -> list[dict]:
    ...
    for r in abiertas:
        if getattr(r, "fecha_entrada", None):
            dias = (ahora - _dt(r.fecha_entrada)).days
